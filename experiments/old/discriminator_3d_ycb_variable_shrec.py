...
from datasets import ycb_shrec_hdf5_discrimination_dataset
from datasets import shrec_h5py_holdout_discrimination_dataset
...
def train(model, train_dataset, test_dataset):
    ...
    with open(ACCURACY_HOLDOUT_VIEWS, "w"):
        print("logging jaccard_error for holdout views")

    with open(ACCURACY_HOLDOUT_MODELS, "w"):
        print("logging accuracy for holdout models")

    with open(TRAINING_PREDICTIONS, "w"):
        print("recording predictions for training set")

    with open(HOLDOUT_MODEL_PREDICTIONS, "w"):
        print("recording predictions for holdout models")

    with open(HOLDOUT_VIEW_PREDICTIONS, "w"):
        print("recording predictions for holdout views")
    ...
    for e in range(NB_EPOCH):
        ...
        for b in range(NB_TRAIN_BATCHES):
            ...
            with open(ACCURACY_FILE, "a") as accuracy_file:
                accuracy_file.write(str(accuracy) + '\n')
            prediction = model._predict(X_batch)


            with open(TRAINING_PREDICTIONS, "a") as training_pred_file:
                training_pred_file.write('target:\n')
                training_pred_file.write(str(Y_batch) + '\n')
                training_pred_file.write('prediction:\n')
                training_pred_file.write(str(prediction) + '\n')
        ...
        average_error = 0
        average_holdout_model_accuracy = 0
        for b in range(NB_TEST_BATCHES):

            """
            X_batch, Y_batch = test_iterator1.next(train=1)
            error, accuracy = model.test(X_batch, Y_batch, accuracy=True)
            #prediction = model._predict(X_batch)
            ...
            error, accuracy = model.test(X_batch, Y_batch, accuracy=True)
            prediction = model._predict(X_batch)
            with open(HOLDOUT_VIEW_PREDICTIONS, "a") as holdout_view_pred_file:
                holdout_view_pred_file.write('target:\n')
                holdout_view_pred_file.write(str(Y_batch) + '\n')
                holdout_view_pred_file.write('prediction:\n')
                holdout_view_pred_file.write(str(prediction) + '\n')


            #binarized_prediction = np.array(prediction > 0.5, dtype=int)
            #holdout_view_jaccard_similarity = numpy_jaccard_similarity(Y_batch,
            #                                              binarized_prediction)
            print('holdout view error: ' + str(error))
            print('holdout view accuracy: '+ str(accuracy))

            #print('jaccard_similarity: ' + str(holdout_view_jaccard_similarity))
            with open(ERROR_HOLDOUT_VIEWS, "a") as error_file:
                error_file.write(str(error) + '\n')
            with open(ACCURACY_HOLDOUT_VIEWS, "a") as acc_file:
            ...
            average_holdout_model_accuracy += accuracy

            print('holdout model error: ' + str(error))
            print('holdout model accuracy: ' + str(accuracy))
            with open(ERROR_HOLDOUT_MODELS, "a") as error_file:
                error_file.write(str(error) + '\n')
            with open(ACCURACY_HOLDOUT_MODELS, "a") as accuracy_holdout_file:
                accuracy_holdout_file.write(str(accuracy) + '\n')

            prediction = model._predict(X_batch)

            with open(HOLDOUT_MODEL_PREDICTIONS, "a") as holdout_model_pred_file:
                holdout_model_pred_file.write('target:\n')
                holdout_model_pred_file.write(str(Y_batch) + '\n')
                holdout_model_pred_file.write('prediction:\n')
                holdout_model_pred_file.write(str(prediction) + '\n')

        average_error /= NB_TEST_BATCHES
        average_holdout_model_accuracy /= NB_TEST_BATCHES
        ...
        if e > 10 and e % 4 ==0 and highest_accuracy <= average_holdout_model_accuracy:
            highest_accuracy = average_holdout_model_accuracy
            print('new highest holdout accuracy: ' + str(highest_accuracy))
            model.save_weights(BEST_WEIGHT_FILE_HOLDOUT_ACCURACY)
...
def get_model():
    ...
    n_out = 2000
    model.add(Dense(2500, n_out, activation='relu', init='he_normal'))


    # output: a vector of size 4000
    # output: a vector of size PATCH_SIZE*PATCH_SIZE*PATCH_SIZE
    model.add(Dense(n_out, 1, init='glorot_normal',
                    activation='sigmoid'))

    optimizer = RMSprop()
    model.compile(loss='weighted_cross_entropy_error', optimizer=optimizer, class_mode = 'binary')
...
def get_dataset(num_shrec_models):
    ...
    train_dataset = ycb_shrec_hdf5_discrimination_dataset.YcbShrecDiscriminationDataset(
    ...
    test_dataset = shrec_h5py_holdout_discrimination_dataset.ShrecHoldoutDataset(
...
if __name__ == "__main__":
    ...
    RESULTS_DIR = '/media/jvarley/46DE3084DE306DF5/shape_completion/' + time.strftime("y%y_m%m_d%d_h%H_m%M") + "/"
    ...
    num_shrec_list = [250]
    #changed range
    for num_shrec_models in num_shrec_list:
        ...
        ACCURACY_FILE = RESULTS_DIR + SUB_DIR + 'accuracy.txt'
        ...
        ACCURACY_HOLDOUT_VIEWS = RESULTS_DIR + SUB_DIR + 'accuracy_holdout_views.txt'
        ACCURACY_HOLDOUT_MODELS = RESULTS_DIR + SUB_DIR + 'accuracy_holdout_models.txt'
        #CURRENT_WEIGHT_FILE = RESULTS_DIR + SUB_DIR + 'current_weights.h5'
        BEST_WEIGHT_FILE = RESULTS_DIR + SUB_DIR + 'best_weights.h5'
        BEST_WEIGHT_FILE_HOLDOUT_ACCURACY = RESULTS_DIR + SUB_DIR + 'best_weights_holdout_accuracy.h5'
        PROFILE_FILE = RESULTS_DIR + SUB_DIR + 'profile.txt'
        TRAINING_PREDICTIONS = RESULTS_DIR + SUB_DIR + 'training_time_predictions.txt'
        HOLDOUT_MODEL_PREDICTIONS = RESULTS_DIR + SUB_DIR + 'holdout_model_predictions.txt'
        HOLDOUT_VIEW_PREDICTIONS = RESULTS_DIR + SUB_DIR + 'holdout_view_predictions.txt'
