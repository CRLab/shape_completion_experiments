...
def test(model, dataset, weights_filepath=BEST_WEIGHT_FILE):
    ...
    #                           "/expected_" + str(i))
    for i in range(batch_size):
        viz.visualize_batch_x_y_overlay(batch_x, batch_y, pred, i=i,
                                        title=str(i))
...
def main():
    for NUM_OBJECTS in [1, 5, 10, 25, 50, 100]:
        ...
        DATA_DIR = '/home/jvarley/3d_conv/keras/examples/' + \
                   'reconstruction_results_novel_view_shrec/' + \
                   str(NUM_OBJECTS) + '/'
        if not os.path.exists(DATA_DIR):
            os.makedirs(DATA_DIR)

        H5_DATASET_FILE_TESTING = '/srv/3d_conv_data/' + \
                                  'shrec_24x24x24_3_25unique_objects.h5'

        # H5_DATASET_FILE = '/srv/3d_conv_data/shrec_24x24x24_2_' + \
        #                   str(NUM_OBJECTS) + '_objects.h5'
        INDICES_FILE = DATA_DIR + 'shrec_recon_indices_relu_' + str(
            NUM_OBJECTS) + '.npy'

        BEST_WEIGHT_FILE = '/home/jvarley/3d_conv/keras/examples/' + \
                           'reconstruction_results_novel_view_shrec/1/' + \
                           '3d_reconstruction_shrec_relu_best_weights.h5'

        print(BEST_WEIGHT_FILE)

        model = get_model()

        test_dataset = shrec_h5py_reconstruction_dataset.ReconstructionDataset(
            hdf5_filepath=H5_DATASET_FILE_TESTING,
            mode='test',
