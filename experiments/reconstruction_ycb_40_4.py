...
def get_model():
    ...
    model.add(Flatten())
    model.add(Dense(nb_filter_out * dim * dim * dim, 200, init='he_normal', activation='relu'))

    model.add(Dense(200, 500, init='he_normal', activation='relu'))

    model.add(Dense(500, PATCH_SIZE * PATCH_SIZE * PATCH_SIZE, init='glorot_normal',
                    activation='sigmoid'))
