...
# this dataset has rgbd images, and returns patches centered around finger
...
class RGBD_HDF5_Dataset(pylearn2.datasets.dataset.Dataset):
    def __init__(self, topo_view_key, y_key, hdf5_filepath, patch_size=72):
        ...
        if not 'num_grasp_type' in self.h5py_dataset.keys():
            ...
            self.h5py_dataset['num_grasp_type'][0] = \
                self.h5py_dataset['grasp_type'][:].max() + 1

        if not 'num_finger_type' in self.h5py_dataset.keys():
            self.h5py_dataset.create_dataset('num_finger_type', (1,))
            self.h5py_dataset['num_finger_type'][0] = \
                self.h5py_dataset['uvd'].shape[1]
    ...
    #     return topo_batch.reshape(topo_batch.shape[-1],
    #                               reduce(mul, topo_batch.shape[:-1]))
...
class HDF5_Iterator():
    def __init__(self, dataset,
                 ...
                 iterator_post_processors=(
                         GaussianNoisePostProcessor(.01, 0, .5),
    ...
    def next(self):
        ...
        #     batch_indices = np.array(range(batch_indices.start, batch_indices.
        #                                    stop))

        # if we are using a shuffled sequential subset iterator
        # then next_index will be something like:
        # array([13713, 14644, 30532, 32127, 35746, 44163, 48490, 49363, 52141,
        #        52216])
        ...
        for i in range(len(finger_indices)):
            ...
            patch = rgbd[u - patch_size / 2.0: u + patch_size / 2.0,
                    v - patch_size / 2.0:v + patch_size / 2.0, :]
