...
class ReconstructionDataset():
    def __init__(self,
                 models_dir="/srv/3d_conv_data/" +
                            "22_model_reconstruction_1000_rand_rot/models/",
                 pc_dir="/srv/3d_conv_data/" +
                        "22_model_reconstruction_1000_rand_rot/pointclouds/",
                 patch_size=72):
        ...
        for model_name in self.model_names:
            model_files = [pc_dir + model_name + "/" + d for d in
                           os.listdir(pc_dir + model_name) if not os.path.isdir(
            ...
            for file_name in file_names:
                ...
                if "_pc.npy" in file_name:
                    ...
                    binvox_model_file = \
                        ...
                        "/optimized_tsdf_texture_mapped_mesh.binvox"

                    self.examples.append(
                        (pointcloud_file, pose_file, binvox_model_file))
...
def create_voxel_grid_around_point(points, patch_center, voxel_resolution=0.001,
    ...
    centered_scaled_points = \
        np.floor((points - patch_center +
                  num_voxels_per_dim / 2 * voxel_resolution) / voxel_resolution)
