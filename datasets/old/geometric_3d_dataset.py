...
class Geometric3DDataset:
    ...
    def __init__(self,
        ...
        # TODO: find a more elegant way rather than hard-coding this
        self.num_labels = 3
...
class BaseGeometric3dIterator():
    ...
    def _generate_solid_figures(self, geometry_types):
        ...
        if self.centered:
            ...
            if self.task == Geometric3DDataset.HALF_COMPLETION_TASK:
                ...
                (x0, y0, z0) = np.random.rand(3) * (
                    (self.patch_size - 1) - 6) + 3

        solid_figures = np.zeros((self.batch_size, self.patch_size, 1,
        ...
        for i in xrange(self.batch_size):
            ...
            if geometry_types[i] == Geometric3DDataset.SPHERE_TYPE:
                # We only iterate through the bounding box of the sphere to
                # check whether voxels are inside the sphere
                radius_squared = radius ** 2
                for z in xrange(z_min, z_max + 1):
                    for x in xrange(x_min, x_max + 1):
                        for y in xrange(y_min, y_max + 1):
                            if (x - x0) ** 2 + (y - y0) ** 2 + (
                                        z - z0) ** 2 <= radius_squared:
                                # inside the sphere
                                solid_figures[i, z, 0, x, y] = 1
            elif geometry_types[i] == Geometric3DDataset.DIAMOND_TYPE:
                # We only iterate through the bounding box of the diamond to
                # check whether voxels are inside the diamond
                for z in xrange(z_min, z_max + 1):
                    for x in xrange(x_min, x_max + 1):
                        for y in xrange(y_min, y_max + 1):
                            if abs(x - x0) + abs(y - y0) + abs(
                                            z - z0) <= radius:
                                # inside the diamond
                                solid_figures[i, z, 0, x, y] = 1
            elif geometry_types[i] == Geometric3DDataset.CUBE_TYPE:
                solid_figures[i, z_min:z_max + 1, 0, x_min:x_max + 1,
                y_min:y_max + 1] = 1
            else:
                raise NotImplementedError
    ...
    def _kinect_scan(self, solid_figures):
        """
        Takes a 5-d boolean numpy array representing batches of 3-d data in
        BZCXY format.
        Returns a 5-d array of the same shape, containing only one "on" z value
        (the one with the lowest index) per each (x, y) pair.
        """
    ...
    def _one_hot(self, labels):
        one_hot_matrix = np.zeros((self.batch_size, self.num_labels),
                                  dtype=np.bool)
...
class Geometric3dClassificationIterator(BaseGeometric3dIterator):
    def next(self):
        # TODO: allow users to specify how they want the classes to be
        # distributed.
        # Currently using same probability for each class
        geometry_types = np.random.randint(0, self.num_labels, self.batch_size)
        labels = self.__one_hot(
            geometry_types)  # self.__one_hot(geometry_types)
...
class Geometric3dKinectCompletionIterator(BaseGeometric3dIterator):
    def next(self):
        labels = self._generate_solid_figures(
